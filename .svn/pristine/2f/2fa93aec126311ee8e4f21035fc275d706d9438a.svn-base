...
<form class="search-form" action="<c:url value="/content/downNoCardInvoice/index" />" method="post" style="overflow: hidden;">
...
<form action="<c:url value="/content/downNoCardInvoice/export" />" target="_blank" method="post" class="export-form">
...
<table class="list-wapper">
	...
	<c:forEach var="item" items="${res.items}" varStatus="status">
		<tr class="list-item">
			...
			<td>
				<c:out value="${item.downRealT1Price}" />
			</td>
			<td>
				<c:out value="${item.status}" /> - <c:out value="${item.statusDesc}" />
			</td>
			<td>
				<a href="<c:url value="/content/downNoCardInvoice/details?id=${item.id}" />" class="choose-link operate-detail" >详情</a>
				<c:if test="${item.status eq '00' or item.status eq '02'}"><a href="<c:url value="/content/downNoCardInvoice/notify/${item.id}" />" class="choose-link operate-check">通知</a></c:if>
			</td>
		</tr>
	</c:forEach>
