...
public class SubMerchantFundAccountDAO extends AbstractPagedDAO<SubMerchantFundAccount> implements ISubMerchantFundAccountDAO {
	...
	public List<SubMerchantFundAccount> findCoinAndAddress() {
		String sql = "select a from SubMerchantFundAccount a";

		TypedQuery<SubMerchantFundAccount> query = this.getEntityManager().createQuery(sql, SubMerchantFundAccount.class);
	...
	@Override
	@Transactional(propagation = org.springframework.transaction.annotation.Propagation.SUPPORTS, readOnly = true)
	public SubMerchantFundAccount findCoinAddress(String address) {
		String sql = "from SubMerchantFundAccount a where a.address = :address ";

		TypedQuery<SubMerchantFundAccount> query = this.getEntityManager().createQuery(sql, SubMerchantFundAccount.class);
		query.setMaxResults(1);
		query.setParameter("address", address);

		List<SubMerchantFundAccount> models = query.getResultList();
	...
	public IPagedList<SubMerchantFundAccount> searchItems(SearchSubMerchantFundAccountData searchData) {
		...
		if (searchData.getCoinName() != null && searchData.getCoinName() != "") {
			conditions.add(new TypeCondition("coinName", "a.coin = :coinName", searchData.getCoinName()));
		}

		if (searchData.getStartDate() != null && searchData.getStartDate() != "") {
			conditions.add(new TypeCondition("startDate", "a.changeDate >= :startDate", searchData.getStartDate().replaceAll("/", "")));
		}
		if (searchData.getEndDate() != null && searchData.getStartDate() != "") {
			conditions.add(new TypeCondition("endDate", "a.changeDate <= :endDate", searchData.getEndDate().replaceAll("/", "")));
		}
	...
	public int changeBalance(SubMerchantFundAccountData diffModel) {
		...
		if (StringUtils.isNotBlank(diffModel.getId())) {
			wheres.add(new TypeCondition("idCons", "a.id = :idCons", diffModel.getId()));
		}
		else {
			// 限定本次更新的用户标识、币类型
