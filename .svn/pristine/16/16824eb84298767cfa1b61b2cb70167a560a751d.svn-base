...
public class DownJerseyManager {
	 ...
	 */
	public Object execute(HttpServletRequest httpRequest, String module, String action, String decodePayload, DownRequestData requestData) throws AbstractWooException {
		IWebService webservice = getWebService(module);

		Method actionMethod = webservice.getActionMethod(action);
		Object request = getRequestData(actionMethod, decodePayload, requestData);
	 ...
	 * @return
	 */
	private Object getRequestData(Method method, String payload, DownRequestData requestData) {
		Type paramsClass = method.getGenericParameterTypes()[0];
		Object request = (Object) JsonUtils.fromJson(payload, paramsClass);
		if (request instanceof IMchServiceData) {
			((IMchServiceData) request).setMchNo(requestData.getMchNo());
		}
		return request;
	}
}
