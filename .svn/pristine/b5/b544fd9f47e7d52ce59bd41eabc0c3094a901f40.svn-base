...
/**
 * 
 * ClassName: PasswayRouteMerchantDAO
 * 
 * @description
 ...
 */
@Service
public class PasswayRouteMerchantDAO extends AbstractPagedDAO<PasswayRouteMerchant> implements IPasswayRouteMerchantDAO {

	@Override
	protected Class<PasswayRouteMerchant> getDomainClass() {
		return PasswayRouteMerchant.class;
	}
	
	@Override
	@Transactional(propagation = org.springframework.transaction.annotation.Propagation.SUPPORTS, readOnly = true)
	public IPagedList<PasswayRouteMerchant> searchItems(SearchPasswayRouteMerchantData searchData) {
		
		StringBuffer sql = new StringBuffer("from PasswayRouteMerchant a");
		...
		TypedQuery<PasswayRouteMerchant> query = this.getEntityManager().createQuery(sql.toString() + " order by a." + searchData.getOrderString(), PasswayRouteMerchant.class);
	...
	@Override
	@Transactional(propagation = org.springframework.transaction.annotation.Propagation.SUPPORTS, readOnly = true)
	public PasswayRouteMerchant findById(String id) {
		PasswayRouteMerchant item = this.findOne(id);
