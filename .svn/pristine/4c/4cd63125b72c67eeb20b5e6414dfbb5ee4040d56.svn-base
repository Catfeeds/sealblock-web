...
<table class="list-wapper">
	...
	<c:set var="downRealT1Price" value="0" />
	...
	<c:forEach var="item" items="${res.items}" varStatus="status">
		...
		<%-- 	<td>
			...
			<td align="right">
				<fmt:formatNumber value="${item.downPayFee}" pattern="#,##0.00#"/>&nbsp;
			</td>
			<td align="right">
				<fmt:formatNumber value="${item.downRealT1Price}" pattern="#,##0.00#"/>&nbsp;
			</td>
			<td align="right">
			...
			<td>
				...
				<c:if test="${item.status eq '00' or item.status eq '02'}"><a href="<c:url value="/admin/downNoCardInvoice/notify/${item.id}" />" class="choose-link operate-check">通知</a></c:if>
			</td>
			
			
			<c:set var="rowNum" value="${rowNum + 1 }" />
			<c:set var="price" value="${price + item.price }" />
			<c:set var="downPayFee" value="${downPayFee + item.downPayFee }" />
			<c:set var="downRealT1Price" value="${downRealT1Price + item.downRealT1Price }" />
	...
	<c:if test="${rowNum gt 0 }">
		<tr class="list-item" style="font-weight: bold;">
			...
			<td align='right'><label style="font-weight: bold; ">
				<fmt:formatNumber type='number' value='${downPayFee}' pattern='0.00' maxFractionDigits='2'/></label>
			</td>
			<td align='right'><label style="font-weight: bold; ">
				<fmt:formatNumber type='number' value='${downRealT1Price}' pattern='0.00' maxFractionDigits='2'/></label>
			</td>
