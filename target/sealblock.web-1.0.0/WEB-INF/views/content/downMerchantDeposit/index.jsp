...
<form class="search-form" action="<c:url value="/content/downMerchantDeposit/index" />" method="post" style="overflow: hidden;">
	...
	<div class="ms-col-fix-100 clear">
		...
		<c:if test="${allowFlag eq 'Y' }">
		<a href="<c:url value='/content/downMerchantDeposit/add' />" style='width: 100px;' class="btn btn-orange choose-link operate-detail" >手动提现</a>
		</c:if>
	</div>
	
</form>
<form action="<c:url value="/content/downMerchantDeposit/export" />" target="_blank" method="post" class="export-form">
	<input type="submit" value="导出" class="btn btn-orange export-btn" ></input><br>
</form>
...
<table class="list-wapper">
	...
	<c:forEach var="item" items="${res.items}" varStatus="status">
		<tr class="list-item">
			...
			<td>
				<%-- <a href="<c:url value="/content/downMerchantDeposit/details?id=${item.id}" />" class="choose-link operate-detail" >详情</a> --%>
				<c:if test="${item.status eq '00' or item.status eq '02'}"><a href="<c:url value="/content/downMerchantDeposit/notify/${item.id}" />" class="choose-link operate-check">通知</a></c:if>
			</td>
		</tr>
	</c:forEach>
