...
public class SysUserController extends BaseController {
	...
	@Transactional(propagation = Propagation.NEVER)
	@RequestMapping(value = "/renewAuth", method = RequestMethod.GET)
	public ModelAndView bindAuth(SystemUserViewData searchData) throws ControllerException {
		ModelAndView mav = new ModelAndView("/admin/user/renewAuth");

		SystemUserViewData item = new SystemUserViewData();
		GoogleAuthenticator gAuth = new GoogleAuthenticator();
		item.setAuthKey(gAuth.createCredentials().getKey());

		mav.addObject("item", item);
		return mav;
	}

	@RequestMapping(value = "renewAuth", method = RequestMethod.POST)
	public ModelAndView doRenewAuth(SystemUserViewData item) {
		if (!org.apache.commons.lang3.StringUtils.isNumeric(item.getAuthCode())) {
			return this.alertFailed("/admin/user/renewAuth", item, "二次验证码需为数字");
		}
		GoogleAuthenticator gAuth = new GoogleAuthenticator();
		if (gAuth.authorize(item.getAuthKey(), Integer.parseInt(item.getAuthCode()))) {
			SystemUser model = systemUserDAO.findOne(this.getUserId());
			model.setAuthKey(item.getAuthKey());
			model.setAuthBindDate(new Date());
			this.systemUserDAO.updateForce(model);

			return new ModelAndView("redirect:/system/user/logout");
		}
		else {
			return this.alertFailed("/admin/user/renewAuth", item, "二次验证提交失败");
		}
	}
		...
		try {
			if (StringUtils.isNotEmpty(itemId)) {
				...
				if (model == null || model.getDeleteStatus() == EnumDeleteStatus.DELETED) {
					...
					systemUserDAO.deleteWithLogical(model);
					message = "删除成功！";
