...
/**
 * ClassName: AgentDownMerchantBalanceController
 * 
 * @description
 * @author Luke
 * @Date Mar 4,2018
 */
@Controller
@RequestMapping("/agent/downMerchantBalance")
public class AgentDownMerchantBalanceController extends BaseController {
	...
	public ModelAndView index(SearchDownMerchantBalanceViewData searchData) throws ControllerException {
		return showIndex(getTemplate("/agent/downMerchantBalance/index"), searchData, null);
	}
	...
	public void export(SearchDownMerchantBalanceViewData searchData, HttpServletResponse response) throws ControllerException {
		...
		if(StringUtils.isEmpty(searchData.getBalanceNo())) {
			if(!SDFFactory.DATE.format(searchData.getStartDate()).equals(SDFFactory.DATE.format(searchData.getEndDate()))) {
				try {
					byte[] content = "不支持跨天导出".getBytes("UTF8");
					response.reset();
	...
	private byte[] formatString(List<DownMerchantBalance> items) throws ParseException {
		...
		headNames.add(new String[] { "账单编号", "账户", "业务类别", "业务流水号", "原账户金额", "变动金额", "账户金额", "登记时间", "原因" });
		List<String[]> fieldNames = new ArrayList<String[]>();
		fieldNames.add(new String[] { "balanceNo", "accTypeFormat", "sourceType", "referTransNo", "lastBalAmt", "diffAmt", "balAmt", "createDate", "remark" });
	...
	public ModelAndView add(DownMerchantBalanceViewData data) throws ControllerException {
		ModelAndView mav = new ModelAndView(getTemplate("/agent/downMerchantBalance/details"));
