...
public class TransService {
	...
	public Response doVerify(@Context HttpServletRequest request, @Context HttpServletResponse httpResponse, String requestString, @PathParam("module") String module, @PathParam("action") String action) {
		...
		try {
			...
			DownMerchantData downMerchant = DownMerchants.getByMchNo(requestData.getMchNo());
			...
			Object result = downJerseyManager.execute(request, "trans" + module.substring(0, 1).toUpperCase() + module.substring(1), action, decodePayload, requestData);
