...
<form action="<c:url value="/transmit/gogateway"/>/${inv.id}" id="submit-form" method="post">
...
<script>
	...
	function check() {
		if (index >= 10) {
			window.location.href = "<c:url value='/transmit/result/${invId}' />";
			return;
		}
		index++;
		$.ajax({
			url: "<c:url value='/transmit/check/${invId}' />",
			type: 'POST',
			dataType: 'json',
			timeout: 30000
		}).done(function(res){
